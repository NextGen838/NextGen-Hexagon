...
def _get_boundary_polygon_from_profile(profile):
    ...
    This does NOT assume that profile.profileLoops[...].profileCurves come
    back already in loop-traversal order, and does NOT assume any curve's
    own start/end direction matches that traversal either - both have been
    observed to vary (simple axis-aligned rectangles or a lone circle just
    happen to line up by coincidence). Instead, all curve segments in the
    loop are tessellated independently and then "stitched" together by
    repeatedly finding, among the not-yet-used segments, whichever one has
    an endpoint that actually connects to the current open end of the
    contour being built - regardless of the original collection order or
    the curve's own parametric direction.
    ...
    # Tessellate every curve in the loop independently first, as a list of
    # (points, geom) segments, without assuming anything about order/direction.
    segments = []
    for profile_curve in outer_loop.profileCurves:
        geom = profile_curve.geometry
        pts = _tessellate_curve3d(geom)
        if len(pts) >= 2:
            segments.append((pts, geom))

    if not segments:
        return [], []

    # Stitch the segments together into a single ordered contour.
    first_pts, first_geom = segments[0]
    polygon = list(first_pts)
    edge_sources = [first_geom] * (len(first_pts) - 1)
    ...
    while remaining and iterations <= max_iterations:
        ...
        best_geom = None
        best_dist = None
        for idx, (pts, geom) in enumerate(remaining):
            d_start = _dist(current_end, pts[0])
            d_end = _dist(current_end, pts[-1])
            if best_dist is None or d_start < best_dist:
                best_dist, best_idx, best_pts, best_geom = d_start, idx, pts, geom
            if d_end < best_dist:
                best_dist, best_idx, best_pts, best_geom = \
                    d_end, idx, list(reversed(pts)), geom

        if best_idx is None:
            break  # should not happen, but never loop forever

        pts_to_add = best_pts
        if _dist(polygon[-1], pts_to_add[0]) < join_tol:
            pts_to_add = pts_to_add[1:]
        for p in pts_to_add:
            edge_sources.append(best_geom)
            polygon.append(p)
...
def _generate_honeycomb(sketch, boundaries, radius, gap, orientation):
    """Generates the honeycomb geometry as SketchLines inside the given
    sketch, filling all of the given boundaries with a single, shared
    hexagon grid (so adjacent boundary areas line up seamlessly instead of
    each getting its own independently-aligned pattern).

    'boundaries' is a list of (boundary_polygon, edge_sources) tuples, one
    per selected boundary profile.

    Returns (full_hex_count, partial_hex_count) summed across all
    boundaries."""

    if not boundaries:
        return 0, 0

    # Combined bounding box across every selected boundary, so a single
    # grid covers all of them at once.
    min_x = min(_bounding_box(poly)[0] for poly, _ in boundaries)
    min_y = min(_bounding_box(poly)[1] for poly, _ in boundaries)
    max_x = max(_bounding_box(poly)[2] for poly, _ in boundaries)
    max_y = max(_bounding_box(poly)[3] for poly, _ in boundaries)
    ...
    # created once, regardless of how many hexagons (or boundaries) touch
    # it.
    drawn_edges = set()
    ...
    try:
        for (cx, cy) in centers:
            ...
            # A hexagon that lies fully inside ANY one of the selected
            # boundaries is drawn whole, immediately, and skipped for the
            # other boundaries.
            fully_inside_boundary = None
            for boundary_polygon, _edge_sources in boundaries:
                if _all_inside(verts, boundary_polygon):
                    fully_inside_boundary = boundary_polygon
                    break

            if fully_inside_boundary is not None:
                for i in range(6):
                    p1 = verts[i]
                    p2 = verts[(i + 1) % 6]
            ...
            # Otherwise, check every boundary individually for a partial
            # (edge-crossing) intersection and clip against each one that
            # applies. A hexagon can legitimately touch more than one
            # boundary if two selected areas are close together.
            any_segment_added = False
            for boundary_polygon, edge_sources in boundaries:
    ...
    def notify(self, args):
        try:
            ...
            selection_input = inputs.addSelectionInput(
                ...
                'Select one or more closed profiles / contours in the '
                'current sketch. Click additional profiles to add them; '
                'ctrl/cmd-click a selected one to remove it.')
            selection_input.addSelectionFilter('Profiles')
            selection_input.setSelectionLimits(1, 0)  # 1 minimum, no maximum
...
def _try_collect_generation_params(cmd_inputs, sketch):
    ...
    boundaries = []
    for i in range(boundary_input.selectionCount):
        selected_entity = boundary_input.selection(i).entity
        ...
        if len(boundary_polygon) < 3:
            continue
        boundaries.append((boundary_polygon, edge_sources))
...
class HoneycombCommandExecuteHandler(adsk.core.CommandEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            if params is None:
                _ui.messageBox('Please select at least one closed boundary '
                                'profile and make sure Hexagon Size / Gap '
                                'are valid before generating.')
                return

            full_count, partial_count = _generate_honeycomb(
                sketch, params['boundaries'],
            ...
            if full_count == 0 and partial_count == 0:
                _ui.messageBox('No hexagons could be generated inside the '
                                'selected boundaries. Please check Hexagon '
                                'Size / Gap.')
            else:
